...
public class command implements CommandExecutor {
				...
				if (sender.hasPermission("deathswap.start")) {
					...
					if (p2 == null) {
						...
						p1.sendMessage("§7[§cDeathSwap§7] The Deathswap between §eyou §7and §e" + p2.getName()
								...
								+ (int) plugin.getConfig().get("minimum time in seconds") + " - "
								+ (int) plugin.getConfig().get("maximum time in seconds") + " seconds §7your positions will be swapped. Kill your §eenemie§7 as fast as possible to win!");
						p2.sendMessage("§7[§cDeathSwap§7] The Deathswap between §eyou §7and §e" + p1.getName()
								+ " §7will start §esoon§7. Every §e" + (int) plugin.getConfig().get("minimum time in seconds") / 60
								+ " - " + (int) plugin.getConfig().get("maximum time in seconds") / 60 + " minutes §7/§e "
								+ (int) plugin.getConfig().get("minimum time in seconds") + " - "
								+ (int) plugin.getConfig().get("maximum time in seconds") + " seconds §7you will be swapped. Kill your §eenemie§7 as fast as possible to win!");

						executer = Bukkit.getScheduler().runTaskLater(plugin, new Runnable() {
			...
			if (args[0].equalsIgnoreCase("stop")) {
				...
				sender.sendMessage(
						"§7[§cDeathSwap§7] Every §e" + (int) plugin.getConfig().get("minimum time in seconds") / 60
								+ " - " + (int) plugin.getConfig().get("maximum time in seconds") / 60 + " minutes §7/§e "
								+ (int) plugin.getConfig().get("minimum time in seconds") + " - "
								+ (int) plugin.getConfig().get("maximum time in seconds") + " seconds §7you will be swapped.");
			} else {
				sender.sendMessage(
						"§7[§cDeathSwap§7] Unknown command. Use §e/deathswap help §7for a list of all commands.");
			}
	...
	public void swap(Player p1, Player p2) {
		...
		main = Bukkit.getScheduler().runTaskLater(plugin, new Runnable() {
			...
			public void run() {
				...
				cdown = Bukkit.getScheduler().runTaskTimer(plugin, new Runnable() {
